"""
Peptide Taste Predictor - Streamlit App (Runtime Training Version)
...
def train_models():
    ...
    try:
        # Load training data
        umami_df = pd.read_csv('data/peptides_umami.csv')
        bitter_df = pd.read_csv('data/peptides_bitter.csv')
        ...
        X_umami = []
        y_umami = []
        for _, row in umami_df.iterrows():
            sequence = row['sequence']
            label = row['label']
            features = extract_features(sequence)
            X_umami.append(list(features.values()))
            y_umami.append(label)
        ...
        X_bitter = []
        y_bitter = []
        for _, row in bitter_df.iterrows():
            sequence = row['sequence']
            label = row['label']
            features = extract_features(sequence)
            X_bitter.append(list(features.values()))
            y_bitter.append(label)
...
if models is None:
    st.error("Failed to train ML models. Please check if data files exist in the 'data/' directory.")
    st.stop()

# Show model info
st.success(f"""
✅ Models trained successfully!
- **Umami Model**: {models['umami']['accuracy']*100:.1f}% accuracy
- **Bitter Model**: {models['bitter']['accuracy']*100:.1f}% accuracy
""")
